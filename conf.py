...
class ConfigBuilder:
    ...
    PI_PULSE_LEN = 36
    _GATES_TO_PULSERS = {"C1": 0, "PAULI": 1, "CNOT": 2, "SWAP": 2, "ISWAP": 2}
    # _GATES_TO_PULSERS = {"C1": 0, "PAULI": 0, "CNOT": 0, "SWAP": 0, "ISWAP": 0}

    def __init__(self, pi_pulse_len=PI_PULSE_LEN, cz_pulse_len=100, bake_cnot=True, bake_swap=True, pulsers_per_qubit=3):
        self.pi_pulse_len = pi_pulse_len
        self.cz_pulse_len = cz_pulse_len
        self.bake_cnot = bake_cnot
        self.bake_swap = bake_swap
        self.config = {}
        self.readout_len = 1000
    ...
    def _place_template(self):
        ...
        'integration_weights': {
            'integW_cosine': {
                'cosine': [1.0] * int(readout_pulse_length),  # [(1.0, readout_pulse_length),]
                'sine': [0.0] * int(readout_pulse_length ),
            },
            'integW_sine': {
                'cosine': [0.0] * int(readout_pulse_length),
                'sine': [1.0] * int(readout_pulse_length ),
            },
        },
    }
