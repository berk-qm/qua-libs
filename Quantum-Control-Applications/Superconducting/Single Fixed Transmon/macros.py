...
class qubit_frequency_tracking:
    ...
    def qua_declaration(self):
        ...
        self.se_vec = declare(fixed, size=2)
    ...
    def two_points_ramsey(self):
        # Scale factor to convert amplitude to frequency change: tau0 is the dephasing time and frequency_sweep_amp is
        # the amplitude of the frequency domain oscillation. The factor 4e-9 is to convert tau from clock cycles to sec.
        # TODO: why no exp(-tau/T2)?
        scale_factor = int(1 / (2 * np.pi * self.tau0 * 4e-9 * self.frequency_sweep_amp))  # in Hz per unit of I, Q or state
        # Average value of the measured quantity (I, state, np.sqrt(I**2+Q**2)...) on both sides of the central fringe.
        assign(self.se_vec[0], 0)  # Left side
        assign(self.se_vec[1], 0)  # Right side
        # Number of averages defined as a power of 2 to perform the average on the FPGA using bit-shifts.
        power_of_2 = 15
        with for_(self.p, 0, self.p < 2**power_of_2, self.p + 1):
            ...
            with for_(self.idx, 0, self.idx < 2, self.idx + 1):
                ...
                assign(self.se_vec[self.idx], self.se_vec[self.idx] + (Cast.to_fixed(self.res) >> power_of_2))
                # Go to the right side of the central fringe
                assign(self.f, self.f + 2 * self.delta)

        # Derive the frequency shift
        assign(self.corr, Cast.mul_int_by_fixed(scale_factor, (self.se_vec[0] - self.se_vec[1])))
