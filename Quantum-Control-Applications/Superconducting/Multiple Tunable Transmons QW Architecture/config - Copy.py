...
def save_results(state: QuAM, filename, figures: List = (), data={}, qua_program=""):
    now = datetime.now()
    ...
    if not os.path.isdir(directory):
        os.mkdir(directory)
    # Save QUA program
    with open(qua_program) as f:
        qua_program = f.read()
        f.close()
    state.results.qua_program = qua_program
    # Save data
    # state.results.data = data
    for q in range(len(data)):
        state.results.data[q].qubit_index = data[q]["qubit_index"]
        state.results.data[q].n_avg = data[q]["n_avg"]
        for key in data[q]["scan_parameters"].keys():
            if key == "axis_1":
                state.results.data[q].scan_parameters.axis_1.name = data[q]["scan_parameters"][key]["name"]
                state.results.data[q].scan_parameters.axis_1.unit = data[q]["scan_parameters"][key]["unit"]
                state.results.data[q].scan_parameters.axis_1.values = list(data[q]["scan_parameters"][key]["values"])
            elif key == "axis_2":
                state.results.data[q].scan_parameters.axis_2.name = data[q]["scan_parameters"][key]["name"]
                state.results.data[q].scan_parameters.axis_2.unit = data[q]["scan_parameters"][key]["unit"]
                state.results.data[q].scan_parameters.axis_2.values = list(data[q]["scan_parameters"][key]["values"])
            elif key == "axis_3":
                state.results.data[q].scan_parameters.axis_3.name = data[q]["scan_parameters"][key]["name"]
                state.results.data[q].scan_parameters.axis_3.unit = data[q]["scan_parameters"][key]["unit"]
                state.results.data[q].scan_parameters.axis_3.values = list(data[q]["scan_parameters"][key]["values"])
            elif key == "axis_4":
                state.results.data[q].scan_parameters.axis_4.name = data[q]["scan_parameters"][key]["name"]
                state.results.data[q].scan_parameters.axis_4.unit = data[q]["scan_parameters"][key]["unit"]
                state.results.data[q].scan_parameters.axis_4.values = list(data[q]["scan_parameters"][key]["values"])
        state.results.data[q].I = [l.tolist() for l in data[q]["I"]]
        state.results.data[q].Q = [l.tolist() for l in data[q]["I"]]
        if "others" in data[q].keys():
            for i in range(len(data[q]["others"])):
                state.results.data[q].others[i].name = data[q]["others"][i]["name"]
                state.results.data[q].others[i].unit = data[q]["others"][i]["unit"]
                state.results.data[q].others[i].values =[l.tolist() for l in data[q]["others"][i]["values"]]
            # Save json
